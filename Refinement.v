...
| Step_Internal : forall st'' acts (int : L.(internal)),
    step L st int st'' ->
...
| Step_Initial_Call : forall st'' acts qb,
    initial_state L st qb st'' ->
    valid_execution_fragment st'' st' acts ->
    valid_execution_fragment st st' (event_inv qb :: acts)
| Step_Final_Return : forall st'' acts rb,
    final_state L st rb st'' ->
    valid_execution_fragment st'' st' acts ->
...
Definition invariant_ind (P : state L -> Prop) :=
  ...
  (forall st st' act,
    P st ->
    initial_state L st act st' ->
    P st') /\
  (forall st st' act,
    P st ->
    at_external L st act st' ->
    P st') /\
  (forall st st' act,
    P st ->
    after_external L st act st' ->
    P st') /\
  (forall st st' act,
    P st ->
    final_state L st act st' ->
    P st').
...
Record fsim_properties (L1 L2 : lts li_null liB) 
    ...
    fsim_match_initial_states:
      forall s1 s1' s2 qb, match_states s1 s2 -> initial_state L1 s1 qb s1' ->
      exists s2', initial_state L2 s2 qb s2' /\ match_states s1' s2';
    fsim_match_final_states:
      forall s1 s1' s2 rb, match_states s1 s2 -> final_state L1 s1 rb s1' ->
      exists s2', final_state L2 s2 rb s2' /\ match_states s1' s2';
    fsim_simulation:
      forall s1 s1' s2 int, match_states s1 s2 -> step L1 s1 int s1' ->
...
Definition fsim_properties_inv_ind (L1 L2 : lts li_null liB)
  ...
  (invariant_ind _ inv) /\
      (forall s1, new_state L1 s1 -> 
      exists s2, new_state L2 s2 /\ match_states s1 s2) /\
      (forall s1 s1' s2 qb, inv s1 -> match_states s1 s2 -> initial_state L1 s1 qb s1' ->
      exists s2', initial_state L2 s2 qb s2' /\ match_states s1' s2') /\
      (forall s1 s1' s2 rb, inv s1 -> match_states s1 s2 -> final_state L1 s1 rb s1' ->
      exists s2', final_state L2 s2 rb s2' /\ match_states s1' s2') /\
      (forall s1 s1' s2 int, inv s1 -> match_states s1 s2 -> step L1 s1 int s1' ->
      exists s2', valid_execution_fragment L2 s2 s2' [] /\ match_states s1' s2')
  .
...
Proof.
  ...
  - destruct Hinv as [Hinvstart [Hinvstep [Hinvinit [Hinvat [Hinvafter Hinvfinal]]]]].
    ...
    specialize (Hrel_trace_step st st'' s2' int Hinvst Hrel H).
  ...
  - destruct Hinv as [Hinvstart [Hinvstep [Hinvinit [Hinvat [Hinvafter Hinvfinal]]]]].
    ...
    assert (inv st'') as Hinvst'' by eauto.
    destruct Hrel_trace as [Hrel_trace_init [Hrel_trace_final Hrel_trace_step]].
    specialize (Hrel_trace_init st st'' s2' qb Hinvst Hrel H).
  ...
  - destruct Hinv as [Hinvstart [Hinvstep [Hinvinit [Hinvat [Hinvafter Hinvfinal]]]]].
    ...
    assert (inv st'') as Hinvst'' by eauto.
    destruct Hrel_trace as [Hrel_trace_init [Hrel_trace_final Hrel_trace_step]].
    specialize (Hrel_trace_final st st'' s2' rb Hinvst Hrel H).
